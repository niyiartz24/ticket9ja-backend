...
async def create_event(event_data: EventCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    new_event = Event(
        name=event_data.name, event_date=event_data.event_date, event_time=event_data.event_time,
        venue=event_data.venue, city=event_data.city, description=event_data.description, 
...
async def update_ticket(ticket_id: int, ticket_data: TicketUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    ticket.ticket_image_path = render_ticket_image(
        event.ticket_design_path or "", ticket.attendee_name, ticket.ticket_type,
        ticket.ticket_id, ticket.qr_code_path, event.name, event.event_date, event.event_time, event.venue
    )
    db.commit()
    db.refresh(ticket)
    return ticket
...
async def validate_ticket(scan_data: ScanRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if ticket.is_used:
        return {
            ...
            "status": "already_used", 
            "ticket": {"attendee_name": ticket.attendee_name, "ticket_type": ticket.ticket_type, "used_at": ticket.used_at.isoformat()}
        }
    ticket.is_used = True
    ...
    event = db.query(Event).filter(Event.id == ticket.event_id).first()
    return {
        "valid": True, "message": "Entry granted", "status": "valid",
        "ticket": {"attendee_name": ticket.attendee_name, "ticket_type": ticket.ticket_type, "ticket_id": ticket.ticket_id, "event_name": event.name}
    }
...
async def get_dashboard_stats(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {
        "total_tickets": total_tickets, "used_tickets": used_tickets,
...
async def get_event_stats(event_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {
        "event_id": event_id, "event_name": event.name, "total_tickets": total_tickets,
        "used_tickets": used_tickets, "remaining_tickets": total_tickets - used_tickets,
        "is_active": event.is_active, "is_locked": event.is_locked
    }
